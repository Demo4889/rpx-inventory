...
local function ConvertQB()
	...
	if qbEvidence then
		...
		for name, items in pairs(oxEvidence) do
			count += 1
			parameters[count] = { "INSERT INTO rpx-inventory (owner, name, data) VALUES ('', ?, ?) ON DUPLICATE KEY UPDATE name = VALUES(name), data = VALUES(data)", {
				name = name,
...
local function Convert_Old_ESX_Property()
	...
	MySQL.prepare.await('INSERT INTO rpx-inventory (owner,name,data) VALUES (?,?,?)', parameters)
